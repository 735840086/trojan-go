...
getData() {
    ...
    if [[ "$(needNginx)" = "no" ]]; then
        ...
        read -p " 请输入Nginx监听端口[10001-65535的一个数字](不懂请直接回车)：" PORT
        [[ -z "${PORT}" ]] && PORT=`shuf -i10001-65535 -n1`
        if [ "${PORT:0:1}" = "0" ]; then
            colorEcho ${BLUE}  " 端口不能以0开头"
            exit 1
        fi

        if [ ${PORT} -lt 10001 ]; then
            colorEcho ${BLUE}  " 端口需在10001-65535之间"
            exit 1
        fi
...
menu() {
    clear
    echo "#############################################################"
    echo -e "#                   ${RED}v2ray简化部署${PLAIN}        #"
    echo -e "# ${GREEN}作者${PLAIN}: hhroot                           #"
    echo -e "# ${GREEN}github地址${PLAIN}: https://github.com/735840086              #"
    echo "#############################################################"

    echo -e "  ${GREEN}1.${PLAIN}   安装V2ray-VMESS+WS+TLS"
    echo " -------------"
    echo -e "  ${GREEN}2.${PLAIN}  更新V2ray"
    echo -e "  ${GREEN}3.  ${RED}卸载V2ray${PLAIN}"
    echo " -------------"
    echo -e "  ${GREEN}4.${PLAIN}  启动V2ray"
    echo -e "  ${GREEN}5.${PLAIN}  重启V2ray"
    echo -e "  ${GREEN}6.${PLAIN}  停止V2ray"
    echo " -------------"
    echo -e "  ${GREEN}7.${PLAIN}  查看V2ray配置"
    echo -e "  ${GREEN}8.${PLAIN}  查看V2ray日志"
    echo " -------------"
